...
test("opens on the scheduled event without redundant navigation", async ({ page }) => {
  ...
  await expect(event).toHaveAttribute("data-state", "upcoming");
  await expect(event.locator("[data-event-state]")).toHaveText("Upcoming");
  await expect(event.locator("time")).toHaveText("13 August 2026");
  const cardOrganizers = event.locator('[data-event-people="organizers"]');
  await expect(cardOrganizers.locator("li")).toHaveCount(1);
  await expect(cardOrganizers).toContainText("Balázs Püspök-Kiss");
...
test("keeps content inside the viewport and exposes the important links", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: "See the event schedule" })).toHaveAttribute("href", "#events");
...
test("links the hero art to the scheduled Budapest event", async ({ page }) => {
  await page.goto("/2026/");
...
test("removes the obsolete postponement notice from every event surface", async ({ page }) => {
  for (const path of ["/2026/", "/2026/events/", "/2026/events/project-kickoff/"]) {
    await page.goto(path);
    await expect(page.locator("[data-notice-banner]")).toHaveCount(0);
    await expect(page.locator("body")).not.toContainText("Kickoff postponed");
    await expect(page.locator("body")).not.toContainText("Venue TBD");
    await expect(page.locator("body")).toContainText("Puzl CowOrKing");
  }

  await page.goto("/2026/events/project-kickoff/");
  await expect(page.getByRole("link", { name: "Reserve a place" })).toHaveAttribute(
    "href",
    "https://luma.com/9b5mxujb"
  );
  await expect(page.locator("body")).not.toContainText("Genesys Hungary office");
  await expect(page.locator('a[href="https://maps.app.goo.gl/4Qk2mNq2eZ7592NPA"]')).toHaveCount(0);
});
...
test("publishes presentation decks responsively as local PDFs", async ({ page }) => {
  await page.goto("/2026/events/project-kickoff/");
  ...
  await expect(presentations.locator(".event-presentation-card__topline > :last-child")).toHaveText([
    "PDF · 15 slides",
    "PDF · 21 slides",
    "PDF · 14 slides"
  ]);
  ...
  const presentationUrls = [
    "/2026/documents/mirzamehdi-karimov-fast-mobile-project-initialization-and-shipping.pdf",
  ...
  for (const href of presentationUrls) {
  ...
  const presentationLayout = await presentations.locator("[data-event-presentations]").evaluate((grid) => ({
    ...
    occupiedColumns: new Set(
      [...grid.querySelectorAll<HTMLElement>(".event-presentation-card")]
        .map((card) => Math.round(card.getBoundingClientRect().left))
    ).size,
    gridOverflow: grid.scrollWidth > grid.clientWidth
  }));
  const viewportWidth = page.viewportSize()!.width;
  let expectedOccupiedColumns: number;
  if (viewportWidth >= 1000) {
    expectedOccupiedColumns = 3;
  } else if (viewportWidth >= 600) {
    expectedOccupiedColumns = 2;
  } else {
    expectedOccupiedColumns = 1;
  }
  expect(presentationLayout.occupiedColumns).toBe(expectedOccupiedColumns);
...
test("navigates from the event grid to MDX details and back", async ({ page }) => {
  ...
  await expect(page.locator(".event-document__hero-copy .eyebrow")).toContainText("Scheduled");
  await expect(facts).toContainText("13 August 2026");
  ...
  const thumbnail = page.locator(".event-document__header").getByRole("img", {
    name: "Ship-a-ton Budapest Kickoff 2026 poster with a rocket-cat astronaut in front of the Puzl CowOrKing building at night"
  });
  ...
  const [thumbnailBox, heroCopyBox, factsBox] = await Promise.all([
...
test("keeps page motion directional and stable from a scrolled route", async ({ page }) => {
  ...
  await page
    .locator('[data-event-card][data-date="2026-08-13"]')
...
test("selects and extrudes the event without changing its layout footprint", async ({ page }) => {
  ...
  const card = page.locator('[data-event-card][data-date="2026-08-13"]');
...
test("gives clickable backdrops the right hover color", async ({ page }, testInfo) => {
  ...
  for (const { backdrop, color } of [
    {
      backdrop: page.getByRole("link", { name: "See the event schedule" }).locator(".button--primary"),
      color: "rgb(23, 19, 38)"
    },
    {
  ...
  const primary = page.getByRole("link", { name: "See the event schedule" });
  await page.mouse.move(0, 0);
